...
const sanitizeMessage = (message: string): string => {
  ...
  sanitized = sanitized.replace(/gpt-4o?(-vision)?/gi, 'AI service');
  sanitized = sanitized.replace(/screenshotapi/gi, 'screenshot service');
  sanitized = sanitized.replace(/urlbox/gi, 'backup screenshot service');
  sanitized = sanitized.replace(/openai/gi, 'AI service');
  sanitized = sanitized.replace(/supabase/gi, 'database service');
  sanitized = sanitized.replace(/claude/gi, 'AI assistant');
  sanitized = sanitized.replace(/anthropic/gi, 'AI service');
  
  // Remove API endpoints and keys
  sanitized = sanitized.replace(/https:\/\/[a-zA-Z0-9.-]+\/[^\s]*/g, '[API_ENDPOINT]');
  sanitized = sanitized.replace(/[a-z0-9]{20,}/gi, '[API_KEY]');
